"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PaymentGatewayFactory = void 0;
const common_1 = require("@nestjs/common");
const transaction_entity_1 = require("../entities/transaction.entity");
const stripe_gateway_service_1 = require("./stripe-gateway.service");
let PaymentGatewayFactory = class PaymentGatewayFactory {
    stripeGatewayService;
    constructor(stripeGatewayService) {
        this.stripeGatewayService = stripeGatewayService;
    }
    getGatewayService(gateway) {
        switch (gateway) {
            case transaction_entity_1.PaymentGateway.STRIPE:
                return this.stripeGatewayService;
            case transaction_entity_1.PaymentGateway.MERCADOPAGO:
                throw new common_1.BadRequestException(`MercadoPago gateway not implemented yet`);
            case transaction_entity_1.PaymentGateway.PAGARME:
                throw new common_1.BadRequestException(`Pagar.me gateway not implemented yet`);
            default:
                throw new common_1.BadRequestException(`Unsupported payment gateway: ${gateway}`);
        }
    }
    getSupportedGateways() {
        return [
            transaction_entity_1.PaymentGateway.STRIPE,
        ];
    }
    isGatewaySupported(gateway) {
        return this.getSupportedGateways().includes(gateway);
    }
};
exports.PaymentGatewayFactory = PaymentGatewayFactory;
exports.PaymentGatewayFactory = PaymentGatewayFactory = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [stripe_gateway_service_1.StripeGatewayService])
], PaymentGatewayFactory);
//# sourceMappingURL=payment-gateway.factory.js.map